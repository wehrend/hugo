...
As we can see in below pictures a voltage is just the difference between two potentials.
In the first example (left) the junction at the bottom is choosen as reference point, as it 
is signaled as ground. So the voltage and also potential difference amounts to 1.5V for U_B0,
but also to 3V for U_A0. Whereas in the example on the right the junction between the battery
cells is choosen as reference point and ground. So the potentialdifference we measure 
here are: U_A0 = 1.5V and U_B0 =-1.5V
...
on a breakout board- the answer is simple scale - it might work for this simple hobbyist example but lack scalibility,costs or reliability.
