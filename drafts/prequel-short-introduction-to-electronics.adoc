...
The table below shows the atomic models of carbon, silicon, and copper conductor. It will become clear in time 
why we picked up those elements. The former two belong to the group of semiconductor, while the latter one is 
an proxy from the group of conductors.

////
With its four electrons on the valence band silicon is 
(in its pure non-doped form) neutral with respect to its condcutivity properties - neither a conductor nor an isolator -. Copper 
on the contrary is excellent conductor, due to its one electron on the valence shell / band. 
////

[width="100%" cols="a,a"]
|=====
| atomic model of carbon | atomic model of silicon | atomic model of copper 
